                    ...
                    when (args[0]) {
                        ...
                        "scoreboard" -> {
                            if (!plugin.pluginStats) {
                                sender.sendMessage("${plugin.prefix}プラグインが${ChatColor.RED}${ChatColor.BOLD}" +
                                        "無効${ChatColor.WHITE}${ChatColor.BOLD}になっています。")
                                return false
                            }
                            plugin.scoreboardStats.putIfAbsent(sender.uniqueId, true)

                            if (plugin.scoreboardStats[sender.uniqueId]!!) {
                                sender.sendMessage("${plugin.prefix}スコアボードを${"" + ChatColor.RED}${ChatColor.BOLD}非表示${ChatColor.WHITE}${ChatColor.BOLD}にしました。")
                                plugin.scoreboardStats[sender.uniqueId] = false
                                return true
                            } else {
                                sender.sendMessage("${plugin.prefix}スコアボードを${"" + ChatColor.GREEN}${ChatColor.BOLD}表示${ChatColor.WHITE}${ChatColor.BOLD}にしました。")
                        ...
                        "stats" -> {
                            ...
                            if (!cmdStats) {
                                sender.sendMessage("" +
                                        ...
                                        else "" + ChatColor.RED + ChatColor.BOLD + "無効"}${ChatColor.WHITE}${ChatColor.BOLD}" +
